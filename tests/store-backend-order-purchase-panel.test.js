...
describe('京东订单详情采购信息区域', () => {
  ...
  it('在订单状态区下方、订单信息区上方插入隔离样式区域，不使用悬浮遮挡', () => {
    ...
    expect(serverSource).toContain("app.get('/api/purchase-orders/by-sales-order/:orderNo'")
    expect(serverSource).toContain('po.owner_id = ? AND (po.sales_order_no = ? OR so.order_id = ?)')
    expect(serverSource).toContain('so.order_id AS linked_sales_order_no')
    expect(serverSource).toContain("COALESCE(NULLIF(po.goods_name, ''), NULLIF(i.product_name, ''), NULLIF(so.product_name, '')) AS goods_name")
    expect(serverSource).toContain("COALESCE(NULLIF(po.goods_image, ''), NULLIF(so.product_image, '')) AS goods_image")
    expect(serverSource).toContain("COALESCE(NULLIF(w.name, ''), NULLIF(so.warehouse_name, '')) AS warehouse_name")
    expect(serverSource).toContain("po.sales_order_no = so.order_id")
    expect(serverSource).toContain('pa.account AS account_name')
    expect(serverSource).toContain('LEFT JOIN user_purchase_accounts upa')
  })
})
