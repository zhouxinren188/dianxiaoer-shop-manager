...
describe('云仓助手第三方服务架构', () => {
  ...
  it('待打印页按当前页订单解析机器并逐单核验入单状态', () => {
    const service = read('server/services/cloud-warehouse-third-party-service.js')
    const route = read('server/routes/cloud-warehouse.js')
    const api = read('src/renderer/src/api/cloudWarehouse.js')
    const renderer = read('src/renderer/src/views/purchase/PurchaseOrder.vue')
    const database = read('server/db.js')
    expect(service).toContain('function buildWarehouseOrderCheckPayload({ requestId, machineCode, orderNo, orderYear })')
    expect(service).toContain("command: 'warehouse.order.check'")
    expect(service).toContain('orderNo: payloadFields.order_no')
    expect(service).toContain('orderYear: payloadFields.order_year')
  ...
  it('云仓逐单查询将采购单、订单号和年份随 request_id 持久化', () => {
    const database = read('server/db.js')
    const service = read('server/services/cloud-warehouse-third-party-service.js')
    expect(database).toContain('scope_order_nos JSON DEFAULT NULL')
    expect(service).toContain('purchaseOrderId: Number(order.id)')
    expect(service).toContain('orderNo: payloadFields.order_no')
    expect(service).toContain('orderYear: payloadFields.order_year')
    expect(service).not.toContain("UPDATE cloud_external_commands SET scope_order_nos = ?")
