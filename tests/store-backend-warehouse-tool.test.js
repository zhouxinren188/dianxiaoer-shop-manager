...
describe('店铺后台分区库存工具', () => {
  ...
  it('为每个店铺后台标签绑定重新注入，并在销毁时清理定时器', () => {
    ...
    expect(browserSource).toContain('this.platformSession.registerPreloadScript({')
    expect(browserSource).toContain("type: 'frame'")
    expect(browserSource).toContain('filePath: this.pagePreloadPath')
  })
})
