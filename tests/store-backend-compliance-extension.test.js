...
describe('店铺后台违规商品清理内置模块', () => {
  ...
  it('京东 AI 自动弹出开关可按店铺持久切换，并且不影响手动打开', () => {
    ...
    expect(cleaner).toContain('if (jdAiAutoCloseDone) return')
  ...
  it('SKU 状态缓存保留七天，到期清理并且当前页优先使用本地记录', () => {
    ...
    expect(annotationFlow).toContain('className = "et-c-info-delete et-c-status-delete"')
    ...
    expect(annotationFlow).toContain('if (!canDelete || !operationCell)')
    expect(annotationFlow).toContain('operationCell.appendChild(controls)')
    expect(cleaner).toContain('.et-c-operation-cell{display:flex!important;align-items:center!important')
    expect(cleaner).toContain('.et-c-status-actions{display:flex!important')
