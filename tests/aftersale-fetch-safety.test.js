...
describe('售后纠纷同步安全机制', () => {
  ...
  it('完整保存首页原始响应并展示物流异常与消费者发票', () => {
    ...
    expect(fetchSource).toContain("const JD_PENDING_INVOICE_PAGE_URL = 'https://shop.jd.com/jdm/finance/consumerInvoice/cinvoiceOrder'")
    expect(fetchSource).toContain('await win.loadURL(JD_PENDING_INVOICE_PAGE_URL)')
    expect(fetchSource).toContain('return { ok: true, response: response }')
    expect(fetchSource).toContain('phase=invoice_page result=loaded')
    expect(fetchSource).toContain("throw new Error(message || '发票接口注入脚本未返回结果')")
