...
describe('淘宝按图搜同款', () => {
  ...
  it('生成可执行的淘宝商品页浮窗和选货源注入脚本', () => {
    ...
    expect(script).toContain('flex:0 0 100%')
    expect(script).toContain("titleBlock.insertAdjacentElement('afterend', row)")
    expect(script).toContain("parent.style.flexWrap = 'wrap'")
    expect(script).toContain("row.style.gridColumn = '1 / -1'")
