...
describe('淘宝按图搜同款', () => {
  ...
  it('生成可执行的淘宝商品页浮窗和工具条选货源注入脚本', () => {
    ...
    expect(script).toContain('__dxe_product_refresh_control__')
    expect(script).toContain('manual-refresh-click')
    expect(script).toContain('window.location.reload();')
    expect(script.match(/window\.location\.reload\(\);/g)).toHaveLength(1)
    expect(script).toContain('sourceRect.left - refreshWidth - 8')
    expect(script).toContain('placeRefreshRow(row)')
    expect(script).toContain('销售规格')
    expect(script).toContain('1300ml单壶')
    expect(script).toContain("image.addEventListener('mouseenter', showImagePreview)")
    expect(script).toContain("image.addEventListener('mouseleave', hideImagePreview)")
    expect(script).toContain("image.style.transform = 'scale(2.45)'")
    expect(script).toContain("imageTransformOrigin: 'left center'")
    expect(script).toContain("transform-origin:' + imageTransformOrigin")
