...
function notifyServer(ver) {
  return new Promise((resolve, reject) => {
    const body = JSON.stringify({
      version: ver,
      changelog: `全量更新 v${ver}: 修复热更新死循环、展开行UI优化、点击行展开绑定商品`
    })
...
conn.on('ready', async () => {
  ...
  try {
    ...
    console.log('\n[5] Verifying...')
    await sleep(5000)
    const check = await execCmd(`curl -s "http://localhost:3001/api/update/check?version=1.3.11&appVersion=1.3.11" 2>&1`)
