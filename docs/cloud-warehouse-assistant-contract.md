...
> **历史方案警告（2026-08-15）**：本文记录的是已经被废弃的“店小二中央执行器控制面”设计，不再是当前开发和部署依据。禁止继续部署本文中的登记、Token、心跳、任务领取、租约、订单映射或结果回传端点。当前有效架构、第三方接口和异常订单流程请以 [云仓助手异常订单流程接手说明](./cloud-warehouse-exception-workflow-handoff.md) 为准。本文仅保留用于追溯旧提交和旧数据库表的由来。
