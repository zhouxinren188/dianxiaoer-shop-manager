...
function notifyServer(ver) {
  return new Promise((resolve, reject) => {
    const body = JSON.stringify({
      version: ver,
      changelog: `全量更新 v${ver}: Cookie持久化修复、商家备注持久化、小眼睛不依赖店铺选择、同步不随页面切换关闭、淘宝采购价获取`
    })
...
conn.on('ready', async () => {
  ...
  try {
    ...
    // Step 3: 上传 exe (大文件)
    console.log('\n[3] Uploading installer (~110 MB, please wait)...')
    await sleep(3000)
    ...
    await sleep(5000)
    const check = await execCmd('curl -s http://localhost:3001/api/update/check?version=1.3.7\\&appVersion=1.3.7 2>&1')
